...
function checkOnline() {
  return new Promise((resolve) => {
    ...
    dns.lookup('launchermeta.mojang.com', (err) => {
      clearTimeout(timer);
      if (resolved) return;
      resolved = true;
      if (err) {
...
async function ensureForge(mcVersion, event) {
  ...
  if (!forgeVersion) {
    ...
    if (!isOnline) {
      throw new Error(`Forge version for MC ${mcVersion} is not pre-mapped and launcher is offline. Please connect to the internet to resolve it.`);
    }
    try {
  ...
  const isOnline = await checkOnline();
  if (!isOnline) {
    throw new Error(`Forge installer for MC ${mcVersion} is not cached and launcher is offline. Please connect to the internet to download it.`);
  }
...
async function ensureFabricApi(mcVersion, profileDir, event) {
  try {
    ...
    const isOnline = await checkOnline();
    if (!isOnline) {
      console.log('[FABRIC API] Launcher is offline. Skipping Fabric API download from Modrinth.');
      event.sender.send('launch-log', 'Launcher is offline. Skipping Fabric API update check.');
      return;
    }
...
async function ensureCustomSkinLoader(mcVersion, profileDir, loaderType, event) {
  try {
    ...
    const isOnline = await checkOnline();
    if (!isOnline) {
      console.log('[SKIN LOADER] Launcher is offline. Skipping CustomSkinLoader download from Modrinth.');
      event.sender.send('launch-log', 'Launcher is offline. Skipping CustomSkinLoader update check.');
      return;
    }
...
function createWindow() {
  ...
  // Always open in maximized / fullscreen bordered mode
  mainWindow.maximize();
...
ipcMain.on('cancel-launch', (event) => {
  ...
  if (activeGameProcess) {
    console.log('[CANCEL] Active game process found, killing...');
    try {
...
ipcMain.handle('get-worlds', async (event, profileId) => {
  try {
    ...
    for (const folderName of folders) {
      ...
      // Detect strictly world-specific in-game screenshot or native world icon
      let screenshotBase64 = null;
      
      // 1. Check world-specific dedicated screenshots folder: saves/<folderName>/screenshots/
      const worldScreenshotsDir = path.join(worldPath, 'screenshots');
      if (fs.existsSync(worldScreenshotsDir)) {
        try {
          const files = fs.readdirSync(worldScreenshotsDir)
            .filter(f => /\.(png|jpe?g|webp)$/i.test(f))
            .map(f => ({ name: f, path: path.join(worldScreenshotsDir, f), mtime: fs.statSync(path.join(worldScreenshotsDir, f)).mtimeMs }))
            .sort((a, b) => b.mtime - a.mtime);
          if (files.length > 0) {
            const ext = path.extname(files[0].name).toLowerCase().replace('.', '') === 'jpg' ? 'jpeg' : 'png';
            screenshotBase64 = `data:image/${ext};base64,` + fs.readFileSync(files[0].path).toString('base64');
          }
        } catch (e) {}
      }

      // 2. Check if any screenshot in screenshots folder was taken strictly during THIS world's active session
      if (!screenshotBase64) {
        const instanceScreenshotsDir = (profileId && profileId !== 'default' && profileId !== '')
          ? path.join(gameDir, 'instances', profileId, 'screenshots')
          : path.join(gameDir, 'screenshots');
        
        if (fs.existsSync(instanceScreenshotsDir)) {
          try {
            // Find world session timestamps from region and level.dat
            let sessionTime = lastPlayed;
            const regionDir = path.join(worldPath, 'region');
            if (fs.existsSync(regionDir)) {
              const regionFiles = fs.readdirSync(regionDir).filter(f => f.endsWith('.mca'));
              if (regionFiles.length > 0) {
                const regionStats = regionFiles.map(f => {
                  try { return fs.statSync(path.join(regionDir, f)).mtimeMs; } catch (e) { return 0; }
                });
                sessionTime = Math.max(sessionTime, ...regionStats);
              }
            }

            const files = fs.readdirSync(instanceScreenshotsDir)
              .filter(f => /\.(png|jpe?g|webp)$/i.test(f))
              .map(f => {
                const fullP = path.join(instanceScreenshotsDir, f);
                return { name: f, path: fullP, mtime: fs.statSync(fullP).mtimeMs };
              })
              .sort((a, b) => b.mtime - a.mtime);
            
            // Match strictly: Screenshot taken during THIS world's session (within 2 hours before session exit / save)
            const matchedScreenshot = files.find(f => {
              const delta = sessionTime - f.mtime;
              // Created up to 10s after session or within 2 hours prior to world save
              return delta >= -10000 && delta <= (2 * 60 * 60 * 1000);
            });

            if (matchedScreenshot) {
              const ext = path.extname(matchedScreenshot.name).toLowerCase().replace('.', '') === 'jpg' ? 'jpeg' : 'png';
              screenshotBase64 = `data:image/${ext};base64,` + fs.readFileSync(matchedScreenshot.path).toString('base64');
            }
          } catch (e) {}
        }
      }

      // 3. World icon fallback (Minecraft auto-creates icon.png on world save for this specific world)
      ...
      worlds.push({
        ...
        sizeBytes,
        icon: iconBase64,
        screenshot: screenshotBase64 || iconBase64
      });
    }
...
ipcMain.handle('backup-world', async (event, { profileId, folderName, worldName }) => {
  try {
    ...
    return new Promise((resolve) => {
      ...
      zip.writeZip(backupZipPath, (err) => {
        if (err) {
          console.error('adm-zip backup error:', err.message);
    ...
    for (const file of files) {
      ...
      if (isDir) {
        ...
        try {
          ...
          if (mcmetaEntry) {
            try {
              const raw = mcmetaEntry.getData().toString('utf8');
              const meta = JSON.parse(raw);
              if (meta && meta.pack) {
                if (meta.pack.description) {
                  description = typeof meta.pack.description === 'string' ? meta.pack.description : (meta.pack.description.text || JSON.stringify(meta.pack.description));
          ...
          if (pngEntry) {
            try {
              iconBase64 = 'data:image/png;base64,' + pngEntry.getData().toString('base64');
            } catch (e) {}
          }
        } catch (err) {
          console.error(`Failed to parse resource pack zip "${file.name}":`, err.message);
        }
      }
...
ipcMain.handle('fetch-mc-versions', async () => {
  ...
  if (isOnline) {
    ...
    console.log('[MANIFEST] Launcher is offline. Skipping online manifest fetch.');
  }

  // Fallback to static list if online fetch failed or was skipped
  if (!result) {
    result = {
  ...
  // Scan local directories for installed versions and merge them
  const localVersions = [];
  try {
    // 1. Scan GAME_DIR/versions
    const mainVersionsDir = path.join(GAME_DIR, 'versions');
    if (fs.existsSync(mainVersionsDir)) {
      const folders = fs.readdirSync(mainVersionsDir);
      for (const folder of folders) {
        const jsonFile = path.join(mainVersionsDir, folder, `${folder}.json`);
        if (fs.existsSync(jsonFile)) {
          localVersions.push(folder);
        }
      }
    }

    // 2. Scan GAME_DIR/instances/*/versions
    const instancesDir = path.join(GAME_DIR, 'instances');
    if (fs.existsSync(instancesDir)) {
      const profiles = fs.readdirSync(instancesDir);
      for (const profile of profiles) {
        const profileVersionsDir = path.join(instancesDir, profile, 'versions');
        if (fs.existsSync(profileVersionsDir)) {
          const folders = fs.readdirSync(profileVersionsDir);
          for (const folder of folders) {
            const jsonFile = path.join(profileVersionsDir, folder, `${folder}.json`);
            if (fs.existsSync(jsonFile)) {
              localVersions.push(folder);
            }
          }
    ...
    console.error('[MANIFEST] Error scanning local versions:', scanErr);
  }

  // Merge, deduplicate, and sort/categorize
  if (localVersions.length > 0) {
    const uniqueLocal = Array.from(new Set(localVersions));
    console.log('[MANIFEST] Scanned local versions:', uniqueLocal);
    for (const v of uniqueLocal) {
      const isSnapshot = v.includes('-') || v.toLowerCase().includes('fabric') || v.toLowerCase().includes('forge') || v.toLowerCase().includes('optifine') || /^\d+w\d+[a-z]$/.test(v);
      if (isSnapshot) {
        if (!result.snapshots.includes(v)) {
          result.snapshots.unshift(v);
        }
      } else {
        if (!result.releases.includes(v)) {
          result.releases.unshift(v);
        }
      }
    }
  }
...
// Extract ZIP via adm-zip (pure JS, cross-platform and non-blocking)
...
ipcMain.handle('get-system-diagnostics', async () => {
  ...
  let javaVersion = 'Java 17 (64-bit)';
  try {
    const runtimeDir = path.join(GAME_DIR, 'runtime');
    if (fs.existsSync(runtimeDir)) {
      const folders = fs.readdirSync(runtimeDir);
      const javaFolders = folders.filter(f => f.startsWith('java-'));
      if (javaFolders.length > 0) {
        const verStr = javaFolders[0].replace('java-', '');
        javaVersion = `Java ${verStr} (Launcher)`;
      } else {
        const javaOutput = execSync('java -version 2>&1').toString();
        const versionMatch = javaOutput.match(/(?:java|openjdk) version "([^"]+)"/i) || javaOutput.match(/build ([^\s\)]+)/i);
        if (versionMatch) {
          javaVersion = `Java ${versionMatch[1]} (System)`;
        }
      }
    } else {
      const javaOutput = execSync('java -version 2>&1').toString();
  ...
  let dirSizeBytes = 1524672000; // default 1.42 GB
  try {
    let size = 0;
    let fileLimit = 1000;
    let filesScanned = 0;
    ...
    async function walk(dir) {
      if (filesScanned > fileLimit) return;
...
// Fetch Ely.by Skin & Cape IPC
ipcMain.handle('fetch-elyby-skin-cape', async (event, username) => {
  try {
    ...
    const capeUrl = `http://skinsystem.ely.by/cloaks/${encodeURIComponent(username)}.png`;
    const metadataUrl = `http://skinsystem.ely.by/textures/${encodeURIComponent(username)}`;
    ...
    console.log(`[ELYBY FETCH] Fetching skin for: ${username}`);
    const skinBase64 = await downloadToBase64(skinUrl);
    
    console.log(`[ELYBY FETCH] Fetching cape for: ${username}`);
    const capeBase64 = await downloadToBase64(capeUrl);
    
    let modelType = 'classic';
    const metadata = await fetchMetadata(metadataUrl);
    if (metadata && metadata.textures && metadata.textures.SKIN) {
      if (metadata.textures.SKIN.metadata && metadata.textures.SKIN.metadata.model === 'slim') {
        modelType = 'slim';
      }
    }
    
    if (!skinBase64) {
      return { success: false, error: 'Skin not found on Ely.by.' };
    }
    ...
    console.error('Error fetching Ely.by skin:', err);
    return { success: false, error: err.message };
  }
});

// mclo.gs Logs Uploader IPC
const querystring = require('querystring');
ipcMain.handle('upload-log', async (event, logText) => {
  return new Promise((resolve) => {
    try {
      const postData = querystring.stringify({ content: logText });
      const reqOpts = {
      ...
      const req = https.request(reqOpts, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            resolve({ success: false, error: 'Failed to parse mclo.gs API response' });
          }
        });
      });
...
// Native Modpack Exporter ZIP helper using adm-zip
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (fs.existsSync(destZipPath)) {
        try { fs.unlinkSync(destZipPath); } catch (e) {}
      }
      const AdmZip = require('adm-zip');
      const zip = new AdmZip();
...
ipcMain.handle('export-profile', async (event, profileId) => {
  try {
    ...
    const profileDir = path.join(GAME_DIR, 'instances', profileId);
    if (!fs.existsSync(profileDir)) {
      throw new Error('Profile folder does not exist');
    }
    
    const config = readLauncherConfig();
    const profile = config.profiles.find(p => p.id === profileId);
    const defaultName = profile ? profile.name : 'modpack';
    const sanitizedName = defaultName.replace(/[^a-zA-Z0-9_\-]/g, '_');
    
    const { canceled, filePath } = await dialog.showSaveDialog(mainWindow, {
      title: 'Export Profile as ZIP Modpack',
      defaultPath: path.join(os.homedir(), 'Downloads', `${sanitizedName}.zip`),
      filters: [
    ...
    await createZip(profileDir, filePath);
    return { success: true, filePath };
  } catch (err) {
    console.error('[EXPORT] Error exporting profile:', err);
    return { success: false, error: err.message };
  }
});
...
ipcMain.handle('save-active-skin-cape', async (event, { username, skinBase64, capeBase64, modelType }) => {
  try {
    ...
    if (skinBase64) {
      ...
      fs.writeFileSync(path.join(cacheDir, `${username}_skin.png`), base64Data, 'base64');
    }
    ...
    if (capeBase64) {
      ...
      const cacheDir = path.join(gameDir, 'skins');
      if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir, { recursive: true });
      fs.writeFileSync(path.join(cacheDir, `${username}_cape.png`), base64Data, 'base64');
    } else {
      const capePath = path.join(capesDir, `${username}.png`);
      ...
      const cacheDir = path.join(gameDir, 'skins');
      const cacheCape = path.join(cacheDir, `${username}_cape.png`);
    ...
    const cacheDir = path.join(gameDir, 'skins');
    if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir, { recursive: true });
    fs.writeFileSync(path.join(cacheDir, `${username}_metadata.json`), JSON.stringify({
      username,
...
ipcMain.handle('load-active-skin-cape', async (event, username) => {
  try {
    const gameDir = GAME_DIR;
    const cacheDir = path.join(gameDir, 'skins');
    const metadataPath = path.join(cacheDir, `${username}_metadata.json`);
    const skinPath = path.join(cacheDir, `${username}_skin.png`);
    const capePath = path.join(cacheDir, `${username}_cape.png`);
...
ipcMain.on('launch-game', async (event, options) => {
  ...
  try {
    ...
    try {
      const activeSkinPath = path.join(gameDir, 'skins', `${username}_skin.png`);
      const activeCapePath = path.join(gameDir, 'skins', `${username}_cape.png`);
    ...
    launcher.on('close', (exitCode) => {
      ...
      if (gameStartTime) {
        ...
        gameStartTime = null;
        console.log(`[PLAYTIME] Session playtime: ${duration} ms`);
        try {
          const config = readLauncherConfig();
          const profile = config.profiles.find(p => p.id === profileId);
          if (profile) {
            profile.playtime = (profile.playtime || 0) + duration;
            writeLauncherConfig(config);
            console.log(`[PLAYTIME] Playtime updated for profile ${profileId}: ${profile.playtime} ms`);
            if (mainWindow) {
              event.sender.send('playtime-updated', { profileId, playtime: profile.playtime });
            }
          }
        } catch (e) {
          console.error('[PLAYTIME] Error saving playtime:', e);
        }
      }
      if (isLaunchCancelled) return;
    ...
    // Check if local version JSON file exists to prevent MCLC from querying Mojang servers.
    if (opts.version.custom) {
      // Modded launch: MCLC needs to resolve the parent vanilla version metadata JSON file.
      // The parent JSON is expected in the custom version directory as `<version.number>.json`
      // (e.g. `versions/fabric-loader-xx-1.20.1/1.20.1.json`).
      const vanillaVersionJson = path.join(profileDir, 'versions', opts.version.number, `${opts.version.number}.json`);
      const customVersionDir = path.join(profileDir, 'versions', opts.version.custom);
      const targetVanillaJson = path.join(customVersionDir, `${opts.version.number}.json`);
      
      if (fs.existsSync(vanillaVersionJson)) {
        try {
          if (!fs.existsSync(customVersionDir)) {
            fs.mkdirSync(customVersionDir, { recursive: true });
          }
          fs.copyFileSync(vanillaVersionJson, targetVanillaJson);
          console.log(`[OFFLINE OPTIMIZATION] Copied parent vanilla JSON to custom version folder: ${targetVanillaJson}`);
        } catch (e) {
          console.error('[OFFLINE OPTIMIZATION] Failed to copy parent vanilla JSON:', e);
        }
      }
    } else {
      // Vanilla launch: Override versionJson directly if it exists.
      const localVersionJson = path.join(profileDir, 'versions', opts.version.number, `${opts.version.number}.json`);
      if (fs.existsSync(localVersionJson)) {
        opts.overrides.versionJson = localVersionJson;
        console.log(`[OFFLINE OPTIMIZATION] Overriding MCLC versionJson directly to: ${localVersionJson}`);
      }
    }
    ...
    if (mainWindow) {
      ...
      // If we keep logs open, we do not hide window
      if (!options.keepLogsOpen) {
        mainWindow.hide();
      }
    }
